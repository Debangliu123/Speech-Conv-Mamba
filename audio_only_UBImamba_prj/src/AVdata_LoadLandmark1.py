...
class AudioandVideoDataset(data.Dataset):
    ...
    def __init__(self, json_dir, batch_size, sample_rate=8000, segment=4.0, cv_maxlen=8.0):
        ...
        #GRID data
        s1Video_json = os.path.join(json_dir, 'speaker1VideomouthYoffset.json')
        s2Video_json = os.path.join(json_dir, 'speaker2VideomouthYoffset.json')



        with open(s1Video_json, 'r') as f:
            s1Video_infos = json.load(f)
        with open(s2Video_json, 'r') as f:
            s2Video_infos = json.load(f)
...
#for Vox/ GRID/LRS2
        ...
        if segment >= 0.0:
            ...
            for i,(_,sample) in enumerate(sorted_mix_infos):
                ...
                _, s1_video_frame = s1Video_infos[i]
                _, s2_video_frame = s2Video_infos[i]
                _, s1_frame = sorted_s1_infos[i]
                _, s2_frame = sorted_s2_infos[i]

                if (s1_video_frame < video_frame_len) or (sample < segment_len) or \
                        (s1_frame < segment_len) or (s2_frame < segment_len) or (s2_video_frame < video_frame_len):
                    drop_utt += 1
                    drop_len += sample
            ...
            while True:
                ...
                while num_segments < batch_size and end < len(sorted_mix_infos):
                    ...
                    # TODO visual data
                    s1_video_utt_len = int(s1Video_infos[end][1])
                    s2_video_utt_len = int(s2Video_infos[end][1])
                    s1_utt_len = int(sorted_s1_infos[end][1])
                    s2_utt_len = int(sorted_s2_infos[end][1])

                    if (s1_video_utt_len >= int(segment * fps_org)) and (utt_len >= segment_len) and \
                            (s1_utt_len >= segment_len) and (s2_utt_len >= segment_len) and (s2_video_utt_len >= int(segment * fps_org)):  # skip too short utt or not not alignment AV frame
                        ...
                        # TODO visual data
                        video_s1.append(s1Video_infos[end])
                        video_s2.append(s2Video_infos[end])
                        assert len(part_mix) == len(part_s1) == len(part_s2) == len(
                            video_s1) == len(video_s2)

                    end += 1
                if len(part_mix) > 0:
                    minibatch.append([part_mix, part_s1, part_s2,video_s1,video_s2,
            ...
            # print('11111',len(self.minibatch))
            # print('11111', self.minibatch)
...
def _collate_fn(batch):
    ...
    # print("aaaaaa",batch)
    assert len(batch) == 1
    mixtures, sources, video_list = load_mixtures_and_sources(batch[0])
    ...
    #TODO: video data
    video_list_pad = pad_list([torch.from_numpy(video).float()
                             for video in video_list], pad_value)
    ...
    return mixtures_pad, ilens, sources_pad, video_list_pad
...
def load_mixtures_and_sources(batch):
    ...
    mix_infos, s1_infos, s2_infos,s1_video_infos ,s2_video_infos,sample_rate, segment_len,video_frame_len = batch
    # for each utterance
    for mix_info, s1_info, s2_info,s1_video_info,s2_video_info in zip(mix_infos, s1_infos, s2_infos,s1_video_infos,s2_video_infos):
        mix_path = mix_info[0]
        s1_path = s1_info[0]
        s2_path = s2_info[0]

        # TODO visual data
        s1_video_path = s1_video_info[0]
        s2_video_path = s2_video_info[0]
        # assert s1_video_info[1] == s2_video_info[1]
        ...
        # TODO visual data
        data1 = pd.read_csv(s1_video_path,sep=' ',header=None)
        data2 = pd.read_csv(s2_video_path,sep=' ',header=None)
        ...
        if segment_len >= 0:
            ...
            # TODO visual data
            #video s1 concatenate vdieo_s2 at feature Dim
            video_list.append(np.concatenate((np.array(data1.values[:video_frame_len]), np.array(data2.values[:video_frame_len])), axis=1))
    return mixtures, sources, video_list
